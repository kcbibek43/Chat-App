...
import { ERROR_MESSAGES, SUCCESS_MESSAGES } from './alert'; // Update path if needed
import Toast from './components/toast';
import Spinner from './components/spinner';

const existingUsernames = ['john', 'alice', 'bob']; // Mock usernames
const existingEmails = ['john@email.com', 'alice@email.com']; // Mock emails
...
const [formData, setFormData] = useState({
  ...
  const isFormValid = isLogin
    ...
    : formData.email.trim() !== '' && formData.password.trim() !== '' && formData.username.trim() !== '';
  ...
  const handleSubmit = (e: React.FormEvent<HTMLButtonElement>) => {
    ...
    if (!isFormValid) {
      setToast(ERROR_MESSAGES.REQUIRED_FIELDS);
      return;
    }

    if (!validateEmail(formData.email)) {
      setToast(ERROR_MESSAGES.INVALID_EMAIL);
      return;
    }

    if (!validatePassword(formData.password) && !isLogin) {
      setToast(ERROR_MESSAGES.WEAK_PASSWORD);
      return;
    }

    if (isLogin) {
      if (!existingEmails.includes(formData.email.trim().toLowerCase())) {
        setToast(ERROR_MESSAGES.USER_NOT_FOUND);
        return;
      }
      if (formData.password !== 'password123') {
        setToast(ERROR_MESSAGES.WRONG_PASSWORD);
        return;
      }
      setToast(SUCCESS_MESSAGES.LOGIN_SUCCESS);
      setTimeout(() => router.push('/chat'), 1200);
    } else {
      if (existingUsernames.includes(formData.username.trim().toLowerCase())) {
        setToast(ERROR_MESSAGES.USERNAME_EXISTS);
        return;
      }
      if (existingEmails.includes(formData.email.trim().toLowerCase())) {
        setToast(ERROR_MESSAGES.EMAIL_EXISTS);
        return;
      }
      setToast(SUCCESS_MESSAGES.SIGNUP_SUCCESS);
  ...
  const toggleMode = () => {
    setIsLogin(!isLogin);
    setFormData({ email: '', password: '', username: '' }); // Clear all fields
    setToast(null);
    setLoading(false); // Also clear spinner if switching modes
  };
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          {isLogin && (
            <div className="text-right">
              <button
                type="button"
                className="text-purple-300 hover:text-purple-200 text-xs sm:text-sm font-medium transition-colors cursor-pointer"
              >
                Forgot password?
              </button>
            </div>
          ...
          <button
            ...
            className={`cursor-pointer w-full bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 text-white font-semibold py-2.5 sm:py-3 px-4 sm:px-6 rounded-lg sm:rounded-xl shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-300 text-sm sm:text-base
